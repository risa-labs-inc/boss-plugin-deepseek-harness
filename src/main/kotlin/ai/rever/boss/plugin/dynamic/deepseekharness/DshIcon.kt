 ...
 *
 * Hand-carried rather than taken from a library: the simple-icons Compose port
 * the host bundles is 1.1.1, which predates DeepSeek and ships no such icon, and
 * the host provides no whale of its own. Note that boss-plugin-docker's icon is
 * also a whale, so these two sit next to each other in the sidebar - this is the
 * DeepSeek silhouette, not a second Docker.
 *
 * The path data is the canonical simple-icons deepseek.svg, verbatim. It is
 * parsed rather than transcribed into PathBuilder calls on purpose: the outline
 * uses elliptical arcs, and hand-converting ~2,000 characters of arc parameters
 * is a silent-corruption risk with no upside. Keeping the string intact also
 * makes a future logo change a copy-paste, and lets it be diffed against
 * upstream.
 *
 ...
 * item's selected and disabled states read wrong.
 */
...
/** Canonical simple-icons deepseek.svg path data (CC0), 24x24 viewBox. */
private val DEEPSEEK_WHALE_PATH: String =
